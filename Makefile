...
ssh-connect:  ## Connect to the instance
	ssh ec2-user@$(IPV4_DNS)

ssh-update-argus:  ## Update Argus remotely
	ssh ec2-user@$(IPV4_DNS) "sudo su -c 'source /usr/local/argus/update_argus.sh'"
...
test-dev:  ## Run tests in the dev container
	docker run -v "$$(pwd)":/usr/local/argus -v argus-data:/usr/local/argus/data -v ~/.aws:/root/.aws -it argus:dev /bin/bash -c 'pytest'
